    ...
    def sample(self, labels=None, cfg_scale=3., n_steps=None, set_seed=False):
        ...
        with torch.no_grad():
            ...
            for t_ in tqdm(range(n_steps)):
                ...
                eps_pred = (1+cfg_scale) * self.eps_model(z_t, labels) - cfg_scale * self.eps_model(z_t)
                    
                #TODO: Get x_t then sample z_t from the reverse process according to 4. and 5. in Algo 2.
                x_t = (z_t - self.diffusion.sigma_lambda(lambda_t) * eps_pred) / self.diffusion.alpha_lambda(lambda_t)
